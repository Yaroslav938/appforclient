...
# ─────────────────────────────────────────────
# УПРАВЛЕНИЕ КЛЮЧАМИ — secrets.toml
# ─────────────────────────────────────────────
SECRETS_DIR = Path(".streamlit")
SECRETS_FILE = SECRETS_DIR / "secrets.toml"

def load_saved_keys():
    """Загрузить сохранённые ключи из secrets.toml"""
    ...
    if SECRETS_FILE.exists():
        ...
        if tomllib:
            with open(SECRETS_FILE, "rb") as f:
                data = tomllib.load(f)
            keys["youtube_api_key"] = data.get("youtube_api_key", "")
            keys["apify_token"] = data.get("apify_token", "")
        else:
            # fallback: читаем вручную
            txt = SECRETS_FILE.read_text()
            for line in txt.splitlines():
                if "youtube_api_key" in line and "=" in line:
                    keys["youtube_api_key"] = line.split("=",1)[1].strip().strip('"').strip("'")
                if "apify_token" in line and "=" in line:
                    keys["apify_token"] = line.split("=",1)[1].strip().strip('"').strip("'")
    return keys

def save_keys(yt_key, apify_key):
    """Сохранить ключи в .streamlit/secrets.toml"""
    SECRETS_DIR.mkdir(exist_ok=True)
    content = f'youtube_api_key = "{yt_key}"\napify_token = "{apify_key}"\n'
...
def search_ytdlp(query, max_results=20):
    """Поиск YouTube через yt-dlp — без API-ключа, полностью бесплатно"""
    try:
        import yt_dlp
    except ImportError:
        return []
        ...
        if use_demo:
            ...
            if "YouTube" in platforms:
                if has_yt_api:
                    ...
                    yt_rows = search_yt_api(yt_api_key, search_query, max_results, region, date_range)
                    if yt_rows:
                        frames.append(pd.DataFrame(yt_rows))
                elif has_ytdlp:
                    st.toast("🆓 Использую yt-dlp (бесплатно)...")
                    yt_rows = search_ytdlp(search_query, max_results)
                    if yt_rows:
                        frames.append(pd.DataFrame(yt_rows))

            if ("Instagram" in platforms or "TikTok" in platforms) and has_apify:
                st.info("Instagram/TikTok: добавьте интеграцию Apify в код (см. вкладку Настройки)")
...
with tab5:
    ...
    """)
    if SECRETS_FILE.exists():
        st.success(f"✅ Файл ключей существует: `{SECRETS_FILE}`")
    else:
        st.info("ℹ️ Файл ключей будет создан после сохранения")
