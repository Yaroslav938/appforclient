...
def fmt_number(n: float | int | None) -> str:
    """Форматирует числа: 1234567 → 1.2М"""
    if n is None:
        return "—"
    try:
        n = float(n)
...
def smart_hashtags(query: str, niche_hashtags: list[str] | None = None) -> list[str]:
    ...
    # Дедуп + до 5 тегов (баланс охвата и стоимости)
    seen = set()
    res = []
    for t in tags:
...
def search_tiktok_rapidapi(
    ...
    """Парсинг TikTok через RapidAPI (scraptik / tiktok-scraper2)."""
    if not rapidapi_key or not query:
        return []

    # Используем tiktok-scraper2 как самый популярный
    headers = {
        "X-RapidAPI-Key": rapidapi_key.strip(),
        "X-RapidAPI-Host": "tiktok-scraper2.p.rapidapi.com",
    }
    r = safe_request(
        "https://tiktok-scraper2.p.rapidapi.com/hashtag/posts",
        params={"hashtag": query.replace("#", "").replace(" ", ""), "count": fetch_limit},
        headers=headers,
    )
    if not r or r.status_code != 200:
        logger.warning("RapidAPI TikTok HTTP=%s", r.status_code if r else "no_response")
        return []
    ...
    rows: list[dict] = []
    for it in items:
        # Разные RapidAPI endpoints имеют немного разный формат — обрабатываем оба
        author = (it.get("author") or {}).get("uniqueId") or (it.get("authorMeta") or {}).get("name", "")
        followers = (it.get("authorStats") or {}).get("followerCount", 0) or 0
        stats = it.get("stats") or it.get("statsV2") or {}
        views = int(stats.get("playCount") or it.get("playCount") or 0)
        likes = int(stats.get("diggCount") or it.get("diggCount") or 0)
        comments = int(stats.get("commentCount") or it.get("commentCount") or 0)
        shares = int(stats.get("shareCount") or it.get("shareCount") or 0)
        vid_id = it.get("id") or it.get("aweme_id", "")
        ts = it.get("createTime", 0) or 0
        try:
            pub_dt = datetime.fromtimestamp(int(ts)) if ts else None
            days_ag = (datetime.now() - pub_dt).days if pub_dt else 30
            published = pub_dt.strftime("%Y-%m-%d") if pub_dt else ""
        ...
        dur_sec = int((it.get("video") or {}).get("duration", 0) or 0)
        score, breakdown = score_video(views, likes + shares * 2, comments, days_ag, followers)

        rows.append({
            "platform": "TikTok",
            "id": vid_id,
            "title": (it.get("desc") or it.get("text") or "")[:120],
            "channel": f"@{author}" if author else "",
            "channel_id": author,
            ...
            "shares": shares,
            "duration_sec": dur_sec, "duration": fmt_duration(dur_sec),
            "thumbnail": (it.get("video") or {}).get("cover", "") or (it.get("video") or {}).get("dynamicCover", ""),
...
with st.sidebar:
    ...
    available = ["YouTube"]
    if has_apify or has_rapidapi or has_instaloader:
...
def load_all_data() -> pd.DataFrame:
    ...
    fetch_limit = max(100, max_results * 3)
        ...
        try:
            if has_yt_api:
                rows = search_youtube_api(
                    st.session_state.youtube_api_key, search_query,
                    fetch_limit=fetch_limit, region=region_code,
                    days=date_range,
                    video_duration=yt_duration_map[yt_duration],
                )
            elif has_ytdlp:
                rows = search_ytdlp(search_query, fetch_limit)
            else:
                rows = []
            if rows:
                frames.append(pd.DataFrame(rows))
            else:
                st.toast("⚠️ YouTube не вернул результатов", icon="⚠️")
        except Exception as e:
            st.warning(f"YouTube: {e}")
        ...
        if has_apify:
            try:
                ig_rows = search_instagram_apify(
                    st.session_state.apify_token, ig_tags, fetch_limit,
                )
            except Exception as e:
                st.warning(f"Instagram (Apify): {e}")
                logger.error("IG Apify error: %s", e)
        # Fallback: Instaloader
        if not ig_rows and has_instaloader:
            st.toast("Пробую Instaloader (бесплатный)...")
            for tag in ig_tags[:2]:  # ограничиваем во избежание бана
                try:
        ...
        if has_apify:
            try:
                tt_rows = search_tiktok_apify(
                    st.session_state.apify_token, tt_tags, fetch_limit,
                )
            except Exception as e:
                st.warning(f"TikTok (Apify): {e}")
                logger.error("TT Apify error: %s", e)
        if not tt_rows and has_rapidapi:
            st.toast("Пробую RapidAPI для TikTok...")
            for tag in tt_tags[:2]:
                try:
                    extra = search_tiktok_rapidapi(
                        st.session_state.rapidapi_key, tag, fetch_limit // 2,
                    )
                    tt_rows.extend(extra)
    ...
    if not frames:
        st.error("Ни один источник не вернул данных. Проверьте подключения или попробуйте другой запрос.")
...
with tab1:
    ...
    for i, row in show_df.iterrows():
        ...
        with col_info:
            ...
            shares_str = f" &nbsp;&nbsp; 🔁 {fmt_number(row.get('shares', 0))}" if row.get("shares") else ""
            subs_str = f"📢 {fmt_number(row.get('subscribers', 0))} подп." if row.get("subscribers") else ""
