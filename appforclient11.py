...
def search_tiktok_rapidapi(
    ...
    for endpoint in TIKTOK_RAPIDAPI_ENDPOINTS:
        ...
        params = {endpoint["params_key"]: clean_query, **endpoint["extra_params"]}
        if "count" in params:
            params["count"] = min(fetch_limit, 30)

        r = safe_request(endpoint["url"], params=params, headers=headers)
        if not r:
            diagnostic_msgs.append(f"{endpoint['host']}: нет ответа")
            continue
        if r.status_code == 401:
            diagnostic_msgs.append(f"{endpoint['host']}: 401 — не подписан на этот API")
            continue
        ...
        if r.status_code == 429:
            ...
            continue
        if r.status_code != 200:
            diagnostic_msgs.append(f"{endpoint['host']}: HTTP {r.status_code}")
            continue

        try:
            data = r.json()
        except Exception:
            diagnostic_msgs.append(f"{endpoint['host']}: невалидный JSON")
            continue

        # Разные эндпоинты — разные ключи ответа
        raw_data = data.get("data") if isinstance(data.get("data"), (dict, list)) else None
        if isinstance(raw_data, dict):
            items = raw_data.get("videos") or raw_data.get("itemList") or raw_data.get("aweme_list") or []
        elif isinstance(raw_data, list):
            items = raw_data
        else:
            items = data.get("itemList") or data.get("aweme_list") or data.get("videos") or []

        if items and len(items) > 0:
            successful_host = endpoint["host"]
            logger.info("RapidAPI TikTok: успех через %s, получено %d", successful_host, len(items))
            break
        diagnostic_msgs.append(f"{endpoint['host']}: пустой ответ")
        ...
        if not has_yt_api and not has_ytdlp:
            ...
            try:
                ...
                if rows:
                    ...
                    diagnostics.append(f"✅ **YouTube ({src_name}):** получено {len(rows)} видео")
                else:
                    diagnostics.append(f"⚠️ **YouTube ({src_name}):** пустой ответ по запросу '{search_query}'. Попробуйте другой запрос или регион.")
            except Exception as e:
                diagnostics.append(f"❌ **YouTube:** {e}")
        ...
        if not has_apify and not has_instaloader:
            ...
            if has_apify:
                try:
                    ...
                    ig_src_logs.append(f"Apify: {len(ig_rows)} видео")
                except Exception as e:
            ...
            if not ig_rows and has_instaloader:
                ...
                for tag in ig_tags[:2]:
                    try:
                        ...
                        ig_src_logs.append(f"Instaloader #{tag}: {str(e)[:80]}")
        ...
        if not has_apify and not has_rapidapi:
            ...
            if has_apify:
                try:
                    ...
                    tt_src_logs.append(f"Apify: {len(tt_rows)} видео")
                except Exception as e:
