...
def search_tiktok_rapidapi(
    ...
    for endpoint in TIKTOK_RAPIDAPI_ENDPOINTS:
        ...
        if endpoint.get("need_challenge_id"):
            ...
            try:
                ...
                if not challenge_id:
                    ...
                    continue
                params = {"challengeId": challenge_id, **endpoint["extra_params"]}
            except Exception as e:
        ...
        # Разные эндпоинты — разные ключи ответа. Пробуем все возможные.
        items = _extract_tiktok_items(data)

        if items and len(items) > 0:
            successful_host = endpoint["host"]
            logger.info("RapidAPI TikTok: успех через %s, получено %d", successful_host, len(items))
            break
        # Даём подсказку по структуре ответа
        top_keys = list(data.keys())[:5] if isinstance(data, dict) else type(data).__name__
        diagnostic_msgs.append(f"{endpoint['host']}: пустой ответ (ключи: {top_keys})")
